...
- Release workflow: GitHub Actions workflow `/.github/workflows/build-release.yml` builds a Debian-compatible Linux bundle via PyInstaller inside `python:3.11-slim-bullseye` (glibc 2.31) invoked with `docker run` on host, triggered by pushes to the `release` branch. The Debian Python image provides a shared libpython required by PyInstaller, then packages `dist/echoendpoint` into `echoendpoint-linux-amd64-<YYYYMMDD>-<shortsha>.tar.gz` and publishes a GitHub Release with tag `release-<YYYYMMDD>-<shortsha>`.
...
- Responsive behavior: desktop keeps the split-pane layout; tablets stack panels with flexible widths; mobile switches to a single-column view with a Requests/Details toggle and a back button in the detail header that swaps between panels while preserving selection state. Responsive rules live in `app/static/css/layout.css` and modal/toast adjustments in `app/static/css/components/`.
- Responsive UI notes: the mobile toggle uses compact padding and 40px tap targets, the hero card heading clamps and wraps to prevent overflow, and the decorative grid on the home page reduces density on small screens. These tweaks are in `app/static/css/layout.css`.
- Mobile tweaks: the dashboard’s Requests/Details toggle shrink-wraps to the button height using mobile-only overrides (inline-flex, no padding/background/border, fixed 40px button height) in `app/static/css/layout.css`, and the home page `.hero-art` is hidden on mobile.
- Mobile layout spacing: the gap between the Requests/Details toggle and the content panels is tightened on mobile by preventing grid row stretching (`.panel { align-content: start; grid-auto-rows: min-content; }`) and clamping adjacency spacing with `#mobile-toggle + .list-panel` in `app/static/css/layout.css`.
