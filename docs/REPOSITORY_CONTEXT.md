...
- Release workflow: GitHub Actions workflow `/.github/workflows/build-release.yml` builds a Debian/Ubuntu-compatible Linux bundle via PyInstaller inside a manylinux_2_28 container (glibc 2.28) invoked with `docker run` on host, triggered by pushes to the `release` branch. The container provides a shared libpython required by PyInstaller, then packages `dist/echoendpoint` into `echoendpoint-linux-amd64-<YYYYMMDD>-<shortsha>.tar.gz` and publishes a GitHub Release with tag `release-<YYYYMMDD>-<shortsha>`.
