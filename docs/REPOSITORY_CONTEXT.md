...
- Release workflow: GitHub Actions workflow `/.github/workflows/build-release.yml` builds a Debian/Ubuntu-compatible Linux bundle via PyInstaller inside a manylinux2014 (glibc 2.17) container invoked with `docker run` on host, triggered by pushes to the `release` branch. It packages `dist/echoendpoint` into `echoendpoint-linux-amd64-<YYYYMMDD>-<shortsha>.tar.gz` and publishes a GitHub Release with tag `release-<YYYYMMDD>-<shortsha>`.
